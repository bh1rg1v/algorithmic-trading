...
def setup_logging(results_dir):
    ...
    log_file = os.path.join(results_dir, 'bt_ml_rf.log')
    logging.basicConfig(
        level=logging.INFO,
...
def main():
    ...
    results_dir = f"backtestresults/ml/{script_name}/rf_run_{timestamp}"
    os.makedirs(results_dir, exist_ok=True)
    
    # Setup logging
    logger = setup_logging(results_dir)
    logger.info(f"Starting Random Forest ML Backtesting Run - {timestamp}")
    ...
    # Save files
    model_path = os.path.join(results_dir, "swing_trading_rf_model.pkl")
    ...
    for stock in stocks:
        ...
        if len(stock_data) > 0:
            ...
            predictions = rf_model.predict(stock_features)
            probabilities = rf_model.predict_proba(stock_features)
    ...
    print("Files created:")
    print("1. swing_trading_rf_model.pkl - Trained model")
    ...
    print("5. bt_ml_rf.log - Execution log")
    print("6. execution_stats.csv - Performance statistics")
    
    logger.info("Random Forest ML Backtesting Run completed successfully")
